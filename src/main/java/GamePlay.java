    ...
    public static void game(){
        ...
        while(!choice.toUpperCase().equals("Q") || deal.getHumanPlayer().getHand().isEmpty() || deal.getComputerPlayer1().getHand().isEmpty()) {
            ...
            do {
                ...
                if (isNumeric(choice)){
                    try {
                        Rules rules = new Rules(deal);
                        if ((Integer.parseInt(choice) < deal.getHumanPlayer().getHand().size() && Double.parseDouble(choice) >= 0)
                                && (rules.isCorrect(deal.getHumanPlayer().getHand().get(Integer.parseInt(choice)))) ) {

// PROBLEM TO RESOLVE!!!
                            deal.putCardOnStock(deal.getHumanPlayer(), Integer.parseInt(choice));
                            if(deal.amountOfSameFigures(deal.getHumanPlayer(),deal.getTopCard())>0){
                                while(deal.amountOfSameFigures(deal.getHumanPlayer(),deal.getTopCard())>0 || !choice.toUpperCase().equals("E")){
                                    Menu.manyCardChoice();
                                    choice = keys.nextLine();
                                    if ((Integer.parseInt(choice) < deal.getHumanPlayer().getHand().size() && Double.parseDouble(choice) >= 0 && !choice.toUpperCase().equals("E"))
                                            && (deal.getTopCard().getFigure().equals(deal.getHumanPlayer().getHand().get(Integer.parseInt(choice)).getFigure()))) {
                                        deal.putCardOnStock(deal.getHumanPlayer(), Integer.parseInt(choice));
                                    }else {
                                        Menu.errorMessage1();
                                    }
